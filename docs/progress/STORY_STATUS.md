...
- [x] STORY-008 — Local dev environment (docker-compose) — depends: STORY-001 — owner: devops-observability-engineer — note: `docker-compose.yml` (postgres:16-alpine/mongo:7/redis:7-alpine/qdrant:v1.11.0, named volumes, healthchecks) already existed from a prior human commit — verified rather than rebuilt: clean `docker compose down -v && up -d` reaches all-`healthy` in ~20s, and a connectivity check against the backend's own Settings/session code (SQLAlchemy/motor/redis.asyncio/httpx) succeeded for all four using `.env.example`'s exact documented defaults. Fixed two real bugs surfaced by that check: `.env.example` had `POSTGRES_PASSWORD` blank while compose defaults to `raas` (now both `raas`, local-dev-only); `backend/requirements.txt` pinned `pymongo==4.8.0` (motor 3.5.1 breaks on pymongo>=4.9's removed `_QUERY_OPTIONS`, blocking `ConfigService`'s Mongo-backed `system_config` resolution). Reran full backend suite post-fix (138 unit + 38 integration) — all green, no regression. `docs/runbooks/local-dev-setup.md` written (docker compose up -> copy .env.example -> alembic upgrade -> uvicorn, plus a port-5432-conflict troubleshooting note found on this dev machine). `docs/features/local-dev-environment.md` written.
...
- [x] STORY-010 — Document upload endpoint — depends: STORY-005, STORY-006, STORY-009 — owner: ingestion-engineer — note: `POST /api/documents/upload` + `GET`/`GET {id}`/`DELETE` CRUD surface in `app/api/documents.py`, org-scoped and auth-gated via `get_current_user`; `validate_upload` (extension allow-list + magic-byte content sniff, registry-based per-extension sniffers) in `app/domain/ingestion/upload_validation.py`; `RepositoryFactory` extended with the `IDocumentRepository` half of the registry (`get_document_repository`), mirroring the existing `IUserRepository` pattern. New `system_config.ingestion.max_upload_size_bytes`/`allowed_extensions` config keys added via `add-config-key`. Tests: unit (`test_upload_validation.py`, extended `test_repository_factory.py`) + integration (`tests/integration/documents/test_upload_endpoint.py`, httpx.AsyncClient + testcontainers Postgres) covering the upload round-trip, 415/413 rejection, auth gating, and cross-org 404 isolation on `GET`/`DELETE`. Implementation was already committed (`8f30cbe`, "story 10 implemntation") before this row/feature doc were closed out — status board and `docs/features/document-upload.md` written retroactively without re-running the test suite, per explicit human instruction.
- [x] STORY-011 — BaseLoader interface + LoaderFactory + TextLoader — depends: STORY-010 — owner: ingestion-engineer — note: `BaseLoader` ABC (`load(file: BinaryIO, *, filename: str) -> RawDocument`) + `RawDocument` dataclass in `app/domain/ingestion/loaders/base.py`; `TextLoader` (UTF-8 decode, `errors="replace"`) in `text_loader.py`; `LoaderFactory.get_loader(file_type)` in `factory.py` using a `_LOADER_REGISTRY` dict keyed by lowercased extension (Open/Closed, mirroring `upload_validation._EXTENSION_SNIFFERS`), reusing STORY-010's `UnsupportedFileTypeError` for unregistered extensions. 15 new unit tests green (124 total backend unit tests green), ruff/mypy clean. Not wired into the upload endpoint or any background job — that's STORY-015's scope.
- [x] STORY-012 — PdfLoader — depends: STORY-011 — owner: ingestion-engineer — note: `PdfLoader` (`app/domain/ingestion/loaders/pdf_loader.py`) using `pypdf.PdfReader`, extracting per-page text joined with `\f` into `RawDocument.content` and per-page `{"page": <1-indexed>, "char_count": <int>}` metadata (`pages`) + `page_count` for later citation `page` metadata (plan §6). Registered as `".pdf": PdfLoader` in `LoaderFactory._LOADER_REGISTRY` — `get_loader()` itself has zero diff (Open/Closed proven). Re-exported alongside STORY-011's names from both `app/domain/ingestion/loaders/__init__.py` and `app/domain/ingestion/__init__.py`. 8 new/changed unit tests (5 new `test_pdf_loader.py` + `test_factory.py` extended with 2 new `.pdf`-resolution tests; its stale STORY-011 test that used `.pdf` as an *unregistered*-extension example was updated to use `.docx` instead, since `.pdf` is now registered) against a hand-built 2-page `fixtures/sample.pdf` (no PDF-authoring library was available, so minimal valid PDF bytes were constructed and round-trip-verified through `pypdf`). 23 tests in the loaders dir, 131 backend unit tests total, all green; ruff/mypy clean. `pypdf==4.3.1` was already pinned in `requirements.txt` (STORY-001) but not yet installed in `backend/.venv` — installed it, no requirements/config change needed. Not wired into the upload endpoint or any background job — that's STORY-015's scope.
- [x] STORY-013 — DocxLoader — depends: STORY-011 — owner: ingestion-engineer — note: `DocxLoader` (`app/domain/ingestion/loaders/docx_loader.py`) using `python-docx`'s `Document`, extracting `paragraph.text` from every paragraph joined with `\n` into `RawDocument.content` plus `{"source", "file_type": ".docx", "char_count", "paragraph_count"}` metadata. Registered as `".docx": DocxLoader` in `LoaderFactory._LOADER_REGISTRY` — `get_loader()` itself has zero diff (Open/Closed proven again). Re-exported alongside existing names from both `app/domain/ingestion/loaders/__init__.py` and `app/domain/ingestion/__init__.py`. 6 new `test_docx_loader.py` tests against a fixture `sample.docx` authored with `python-docx` itself (now available as a dependency, unlike STORY-012's hand-built PDF bytes); `test_factory.py`'s STORY-012-era `.docx`-as-unregistered-example test was updated to two new `.docx`-resolution tests plus a re-targeted `.xlsx`-unregistered test. 30 tests in the loaders dir, 138 backend unit tests total, all green; ruff/mypy clean. `python-docx==1.1.2` was already pinned in `requirements.txt` (STORY-001) but not yet installed in `backend/.venv` — installed it, no requirements/config change needed; no real `.env` touched. Not wired into the upload endpoint or any background job — that's STORY-015's scope.
